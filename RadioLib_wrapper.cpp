...
RadioLib_Wrapper<T>::RadioLib_Wrapper(RADIO_CONFIG radio_config)
{
    ...
    {
        Serial.println("Initialization failed with status code: " + String(state.action_status_code));
        return;
    }
...
bool RadioLib_Wrapper<T>::configure_radio(RADIO_CONFIG radio_config)
{
    ...
    {
        Serial.println("Frequency is invalid: " + String(radio_config.FREQUENCY));
        return false;
    };

    if (radio.setOutputPower(radio_config.TXPOWER) == RADIOLIB_ERR_INVALID_OUTPUT_POWER)
    {
        Serial.println("Transmit power is invalid: " + String(radio_config.TXPOWER));
        return false;
    };

    if (radio.setSpreadingFactor(radio_config.SPREADING) == RADIOLIB_ERR_INVALID_SPREADING_FACTOR)
    {
        Serial.println("Spreading factor is invalid: " + String(radio_config.SPREADING));
        return false;
    };

    if (radio.setCodingRate(radio_config.CODING_RATE) == RADIOLIB_ERR_INVALID_CODING_RATE)
    {
        Serial.println("Coding rate is invalid: " + String(radio_config.CODING_RATE));
        return false;
    };

    if (radio.setBandwidth(radio_config.SIGNAL_BW) == RADIOLIB_ERR_INVALID_BANDWIDTH)
    {
        Serial.println("Signal bandwidth is invalid: " + String(radio_config.SIGNAL_BW));
        return false;
    };

    if (radio.setSyncWord(radio_config.SYNC_WORD) == RADIOLIB_ERR_INVALID_SYNC_WORD)
    {
        Serial.println("Sync word is invalid: " + String(radio_config.SYNC_WORD));
        return false;
    };
    ...
    if (radio_config.rf_switching == RADIO_CONFIG::RF_SWITCHING::GPIO)
    {
        ...
        {
            Serial.println("RF switching setup is invalid: GPIO");
            return false;
        }
    }
    else if (radio_config.rf_switching == RADIO_CONFIG::RF_SWITCHING::DIO2)
    {
        ...
        {
            Serial.println("RF switching setup is invalid: DIO2");
            return false;
        }
    }
...
bool RadioLib_Wrapper<T>::transmit(String msg)
{
    ...
    // If transmit failed, print Serial.println
    if (state.action_status_code != RADIOLIB_ERR_NONE)
    {
        Serial.println(" Starting transmit failed with status code:" + String(state.action_status_code));
        return false;
    }
...
bool RadioLib_Wrapper<T>::receive(String &msg, float &rssi, float &snr)
{
    ...
    if (state.action_type == State::Action_Type::RECEIVE)
    {
        ...
        if (state.action_status_code != RADIOLIB_ERR_NONE)
        {
            Serial.println("Receiving failed with status code: " + String(state.action_status_code));
        }
...
bool RadioLib_Wrapper<T>::test_transmit()
{
    ...
    {
        Serial.println("Test transmission failed. Setting radio as not initialized!");
